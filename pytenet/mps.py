...
class MPS:
    ...
    def __init__(self, qd: Union[Sequence[Sequence[int]],Sequence[int]], qD: Sequence[Sequence[int]], fill=0.0, rng: np.random.Generator=None):
        ...
        if isinstance(fill, (int, float, complex)):
            self.A = [np.full((d[i], D[i], D[i+1]), fill) for i in range(len(D)-1)]
        elif fill == 'random':
            # random complex entries
            if rng is None:
                rng = np.random.default_rng()
            self.A = [crandn((d[i], D[i], D[i+1]), rng) / np.sqrt(d[i]*D[i]*D[i+1]) for i in range(len(D)-1)]
        elif fill == 'postpone':
            self.A = (len(D) - 1) * [None]
        else:
        ...
        if fill != 'postpone':
            ...
            for i in range(len(self.A)):
                mask = qnumber_outer_sum([self.qd[i], self.qD[i], -self.qD[i+1]])
    ...
    def orthonormalize(self, mode='left'):
        ...
        if mode == 'left':
            for i in range(len(self.A) - 1):
                self.A[i], self.A[i+1], self.qD[i+1] = local_orthonormalize_left_qr(self.A[i], self.A[i+1], self.qd[i], self.qD[i:i+2])
            # last tensor
            self.A[-1], T, self.qD[-1] = local_orthonormalize_left_qr(self.A[-1], np.array([[[1]]]), self.qd[i], self.qD[-2:])
        ...
        if mode == 'right':
            for i in reversed(range(1, len(self.A))):
                self.A[i], self.A[i-1], self.qD[i] = local_orthonormalize_right_qr(self.A[i], self.A[i-1], self.qd[i], self.qD[i:i+2])
            # first tensor
            self.A[0], T, self.qD[0] = local_orthonormalize_right_qr(self.A[0], np.array([[[1]]]), self.qd[i], self.qD[:2])
            # normalization factor (real-valued since diagonal of R matrix is real)
            assert T.shape == (1, 1, 1)
            nrm = T[0, 0, 0].real
            if nrm < 0:
...
def add_mps(mps0: MPS, mps1: MPS, alpha=1) -> MPS:
    ...
    if L == 1:
        ...
        # consistency check
        assert is_qsparse(mps.A[0], [mps.qd[0], mps.qD[0], -mps.qD[1]]), \
        ...
        # consistency check
        for i in range(1, L):
            assert is_qsparse(mps.A[i], [mps.qd[i], mps.qD[i], -mps.qD[i+1]]), \
                'sparsity pattern of MPS tensor does not match quantum numbers'
    return mps
